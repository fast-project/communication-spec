...
Diskussion
    ...
    Einheitlicher task/result Name: "task: vm", "result: vm " //J: Wichtiger wäre wohl jeder Nachricht eine eindeutige ID zu geben, die beim Antworten wieder mitgeschickt wird.
...
Subscriber
    ...
    Migration-Framework
        fast/migfra//task Entgegennehmen von Anfragen des Schedulers
        fast/pscom///response Antwort vom entsprechenden Prozess, dass alle Verbindungen herunter gefahren wurden
    Agent
        fast/migfra//result Aenderungen im Schedule des betreffenden nodes (z.B. neuer Job)
        fast/agent//task Anfragen des Schedulers an den entsprechenden Agenten (z.B. start/stop monitoring)
    pscom
        fast/pscom///request Entgegennehmen von Anfragen des Migrationsframeworks
    Anwendungen //J: Von mir, aber noch nicht ganz klar ob das so passt
        fast/migfra//task Überwacht um mögliche Migration festzustellen <- so aber keine Möglichkeit einzugreifen
        fast/application///task <- nicht über VMs hinweg identisch / =VM name? Entgegennehmen der Anfragen des Agenten / Schedulers / MigFra

Publisher

    Scheduler
        fast/migfra//task Starten, Stoppen und Migrieren von VMs
        fast/agent//task starten/stoppen des Monitorings; Konfiguration der KPIs
    Migration-Framework
        fast/migfra//result VM gestartet/gestoppt/migriert
        fast/pscom///request Herunterfahren von pscom Verbindungen
    Agent
        fast/agent//status Anmeldung des Agenten beim Scheduler
    pscom
        fast/pscom///response Information an Migrationsframework, dass Verbindungen herunter gefahren wurden

    Anwendungen //J: Von mir, aber noch nicht ganz klar ob das so passt //* fast/application///status <- nicht über VMs hinweg identisch / =VM name?
        fast/applicaiton//status Status Informationen der Anwendung
        fast/application//KPIs
...
Der Scheduler meldet sich beim Agenten und liefert eine initiale Konfiguration.
    ...
    topic: fast/agent//task/init_agent
    Payload
...
Der Agent soll aufhören die Anwendung zu überwachen.
    ...
    topic: fast/agent//task/stop_monitoring
    Payload task: stop monitoring job-description: job-id: <job id> process-id: <process id of the vm>
...
vm-configurations:
  ...
  - ..
    ...
    Erwartetes Verhalten: VMs werden auf dem entsprechenden Host gestartet.
...
list:
  ...
  - ..
    ...
    Implementierung: Über mqtt_publish in Startup Skript der VM. Hierbei gibt es die folgenden Möglichkeiten:
...
supported_KPI :  
  ...
  - ..
    ...
    Erwartetes Verhalten: Application is migratable be default. However this can be changed by setting 'migration' to 'no' in the status message. Also application at start up can declare the list of KPIs that it is welling to provide. #### Application KPI
    topic: fast/application//KPIs
    payload:
